...
PLAN_MODE_ADDENDUM = (
    ...
    "can't act on anything. create_skill, remember, run_python, and any "
    "skill that writes files, runs a subprocess, or hits the network are "
    "temporarily hidden from your tool list - they still exist and will "
    "work normally once plan mode ends, they are just withheld right now "
    "so you can't take any mutating/action-taking step. Don't conclude a "
    "tool is missing or needs to be (re)created just because you can't see "
    "it in plan mode. Instead, investigate as needed and respond with a "
    "clear, concrete plan of what you would do. Wait for the user to leave "
    "plan mode (they'll type /auto) before anything in the plan gets "
    "executed."
)
